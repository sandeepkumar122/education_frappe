...
def get_students(academic_year, group_based_on, academic_term=None, program=None, batch=None, student_category=None, course=None):
	...
	student=frappe.db.sql(""" select name as student,concat(first_name,' ',last_name) as student_name from `tabStudent Applicant` where application_status=%s and program=%s""",('In-Merit',program),as_dict=1)
